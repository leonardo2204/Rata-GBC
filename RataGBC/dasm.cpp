...
int dasm::DAsm(FILE *F,QListWidget *texto,int &Counter) {
	...
	switch (I) {
	case 0xCB: if ((I = getc(F)) < 0) return (0);
		strcpy(S, MnemonicsCB[I]);
		Counter++;
		break;
	case 0xED: if ((I = getc(F)) < 0) return (0);
		strcpy(S, MnemonicsED[I]);
		Counter++;
		break;
	case 0xFD: if ((I = getc(F)) < 0) return (0);
		strcpy(S, MnemonicsXX[I]);
		if (P = strchr(S, '%')) *P = 'Y';
		Counter++;
		break;
	case 0xDD: if ((I = getc(F)) < 0) return (0);
		strcpy(S, MnemonicsXX[I]);
		if (P = strchr(S, '%')) *P = 'X';
		Counter++;
		break;
